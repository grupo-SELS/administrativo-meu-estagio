...
export default function AgendamentoEstagio() { // NOSONAR
    ...
    const buildEstagioCSVBlock = (estagio: Estagio, index: number): string[] => {
        ...
        if (professorAtribuido) {
            ...
            lines.push(`${professorAtribuido.nome},${professorAtribuido.matricula || 'N/A'},${professorAtribuido.polo || 'resende'},${horarioProfessor}`);
        } else {
            lines.push('Nenhum professor atribuído,,,');
        }

        lines.push('', 'Alunos no Estágio:', 'Nome,Matrícula,Polo,Horário');

        if (alunosAtribuidos.length > 0) {
            for (const aluno of alunosAtribuidos) {
                const horarioAluno = aluno.horario || estagio.horarios?.join(' - ') || '';
                lines.push(`${aluno.nome},${aluno.matricula || 'N/A'},${aluno.polo || 'voltaredonda'},${horarioAluno}`);
            }
        } else {
            lines.push('Nenhum aluno atribuído,,,');
        }
        lines.push('');
    ...
    const downloadCSV = (csvLines: string[]): void => {
    ...
    const handleExportarCSV = () => {
        try {
            ...
            const csvLines = buildCSVHeader();
            for (const estagio of estagios) {
                const index = estagios.indexOf(estagio);
                csvLines.push(...buildEstagioCSVBlock(estagio, index));
            }
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6" style={{ overflow: 'visible' }}>
                        ...
                        <section className="bg-gradient-to-br from-gray-800 via-gray-800 to-gray-900 rounded-2xl shadow-2xl border border-gray-700/50 flex flex-col" style={{ overflow: 'visible' }}>
                            ...
                            {showProfessores ? (
                                <>
                                    {/* sonarqube-disable-next-line */ loadingProfessores ? (
                                        <div className="text-gray-300 text-center py-8">Carregando professores...</div>
                                    ) : /* sonarqube-disable-next-line */ erroProfessores ? ( // NOSONAR
                            ...
                            ) : /* sonarqube-disable-next-line */ loadingAlunos ? ( // NOSONAR
                                <div className="text-gray-300">Carregando alunos...</div>
                            ) : /* sonarqube-disable-next-line */ erroAlunos ? ( // NOSONAR
                        ...
                        <section className="bg-gradient-to-br from-gray-800 via-gray-800 to-gray-900 rounded-2xl shadow-2xl border border-gray-700/50 flex flex-col" style={{ overflow: 'visible' }}>
                            ...
                            {/* sonarqube-disable-next-line */ loadingEstagios ? (
                                <div className="text-gray-300 text-center py-8">Carregando estágios...</div>
                            ) : /* sonarqube-disable-next-line */ erroEstagios ? ( // NOSONAR
                            ...
                            <div className="flex flex-col gap-6" style={{ overflow: 'visible' }}>
                                ...
                                {currentEstagios.map((estagio: Estagio) => (
                                    <button
                                        key={estagio.id}
                                        className={`relative rounded-xl p-5 shadow-xl cursor-pointer transition-all duration-300 border-2 text-left ${
                                        ...
                                        aria-pressed={estagioSelecionado === estagio.id}
                                    >
                                        {estagioSelecionado === estagio.id && (
            ...
            {modalAtribuidosAberto && estagioVisualizando && (
                ...
                >
                    ...
                    <div // NOSONAR
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Escape') setModalAtribuidosAberto(false);
                        }}
                    >
            ...
            {modalEditarAberto && estagioEditando && (
                ...
                >
                    ...
                    <div // NOSONAR
                        ...
                        className="bg-gray-800 rounded-2xl shadow-2xl border border-gray-700 max-w-2xl w-full max-h-[80vh] overflow-hidden"
                        onKeyDown={(e) => {
                            if (e.key === 'Escape') setModalEditarAberto(false);
                        }}
                    >
                        <div className="bg-gradient-to-r from-blue-900 to-blue-800 px-6 py-4 flex items-center justify-between border-b border-gray-700">
                            <div>
                        ...
                        <div className="p-6 overflow-y-auto max-h-[calc(80vh-180px)]">
                            <div className="space-y-4">
                                <div>
                                    <label htmlFor="local-estagio" className="block text-sm font-medium text-gray-300 mb-2">Local do Estágio</label>
                                    <input
                                        id="local-estagio"
                                        type="text"
                                ...
                                <div>
                                    <label htmlFor="area-estagio" className="block text-sm font-medium text-gray-300 mb-2">Área</label>
                                    <input
                                        id="area-estagio"
                                        type="text"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="horario-inicio" className="block text-sm font-medium text-gray-300 mb-2">Horário Início</label>
                                        <input
                                            id="horario-inicio"
                                            type="time"
                                    ...
                                    <div>
                                        <label htmlFor="horario-fim" className="block text-sm font-medium text-gray-300 mb-2">Horário Fim</label>
                                        <input
                                            id="horario-fim"
                                            type="time"
                                ...
                                <div>
                                    <label htmlFor="vagas-disponiveis" className="block text-sm font-medium text-gray-300 mb-2">Vagas Disponíveis</label>
                                    <input
                                ...
                                <div>
                                    <label htmlFor="status-estagio" className="block text-sm font-medium text-gray-300 mb-2">Status</label>
                                    <select
                                        id="status-estagio"
