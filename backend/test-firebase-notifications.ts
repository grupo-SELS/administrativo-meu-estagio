/**
 * Script de teste para verificar a integração com Firebase Notifications
 * Execute com: npx ts-node test-firebase-notifications.ts
 */
...
async function testFirebaseNotifications() {
  ...
  try {
    ...
    const testDoc = await db.collection('notifications').limit(1).get();
    ...
    const docRef = await db.collection('notifications').add(testComunicado);
    console.log(`✅ Comunicado criado com ID: ${docRef.id}`);
    
    console.log('\n🔍 Buscando comunicado criado...');
    const doc = await db.collection('notifications').doc(docRef.id).get();
    if (doc.exists) {
