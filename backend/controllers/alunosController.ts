...
export class AlunosController {
  ...
  private async validateCPFForUpdate(cpf: string, currentCPF: string, alunoId: string): Promise<{ valido: boolean; cpfSanitizado: string; erro?: string }> {
    ...
    const cpfExistente = await cpfJaExiste(db, resultadoCPF.cpfSanitizado, 'alunos', alunoId);
    if (cpfExistente) {
  ...
  private processTags(tags: any, currentTags: any[]): any[] {
  ...
  private processImages(imagens: any, existingImages: any): any[] {
    let processedExisting: any[] = [];
  ...
  async editar(req: Request, res: Response): Promise<void> {
    try {
      ...
      const cpfValidation = await this.validateCPFForUpdate(cpf, (currentData.cpf as string), id);
      if (!cpfValidation.valido) {
        res.status(cpfValidation.erro?.includes('duplicado') ? 409 : 400).json({ error: cpfValidation.erro });
        return;
      }

      const processedTags = this.processTags(tags, (currentData.tags as any[]) || []);
